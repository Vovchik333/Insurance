...
public class SortContractsByRiskLevelController {
    ...
    void clickMouse(MouseEvent event) {
        ...
        if(action != null){
            ...
            ArrayList<String> arrayList = ConnectorDB.showTable("SELECT * FROM Obligations WHERE derivative_id = "
                    + der_id + " ORDER BY risk_lvl DESC", 1);
            list.addAll(arrayList);
            listInformation.setItems(list);
            Logger.getGlobal().info("contracts are successfully sorted by risk level.");
        }
    }
}
